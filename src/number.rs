...
impl Number for IBig {
    ...
    fn mod_pow(a: &Self, e: &IBig, m: &IBig) -> IBig {
        if e.is_zero() {
            return ibig!(1);
        }
        let mut ex = e.clone();
    ...
    fn exgcd(&self, b: &IBig) -> (IBig, IBig, IBig) {
    ...
    fn mod_sqrt(a: &Self, p: &Self) -> IBig {
        let one = ibig!(1);
        let p1 = p - 1i32;
    ...
    fn gen_rand(min: &IBig, max: &IBig) -> IBig {
...
impl Number for BigInt {
    ...
    fn mod_pow(a: &Self, e: &BigInt, p: &BigInt) -> BigInt {
        a.modpow(e, p)
    }

    fn exgcd(&self, b: &BigInt) -> (BigInt, BigInt, BigInt) {
        let (mut q, mut r, mut s, mut t);
        let (mut x, mut y) = (self.clone(), b.clone());
    ...
    /// mod inverse n^(-1) mod p
    fn mod_inv(a: &Self, p: &BigInt) -> BigInt {
        // assert!(n * modinv(n, p) == 1);
        let mut m = a.clone();
    ...
    /// Tonelli's Algorithm
    #[allow(clippy::many_single_char_names)]
    fn mod_sqrt(a: &Self, p: &BigInt) -> BigInt {
        let mut rng = rand::thread_rng();
        ...
        while k < s {
            ...
            let e = &BigInt::from(2i32).modpow(&(&s - &k - 1u32), p);
            let d = b.modpow(e, p);
            if d == p - 1u32 {
                r = &r * &c % p;
            }
    ...
    fn gen_rand(min: &BigInt, max: &BigInt) -> BigInt {
        let mut rng = rand::thread_rng();
